...
import core
from core import Env, State, GAME_ID, Info
from envs.Poker.renderer import create_board_str
...
class PokerEnv(Env):
    ...
    def reset(self, num_players: int, seed: Optional[int] = None):
        ...
        self.state = State(num_players=num_players, min_players=2, max_players=15)
    ...
    def step(self, action: str) -> Tuple[bool, Info]:
        ...
        if gs["round"] < 1 or gs["round"] > self.num_rounds or gs["betting_round"] < 0 or gs["betting_round"] > 3:
            ...
            self.state.add_observation(from_id=GAME_ID, to_id=-1, message=err_msg)
    ...
    def _apply_action(self, player_id: int, action_type: str, bet_amount: Optional[int]):
        ...
        Now with proper tracking of the last bettor and bet round completion.
        """
        gs = self.state.game_state
        ...
        if action_type == "fold":
            ...
            self.state.add_observation(from_id=GAME_ID, to_id=-1, message=f"Player {player_id} folds.")
            ...
            if gs["current_bet"] > gs["player_bets"][player_id]:
                ...
                self.state.add_observation(from_id=GAME_ID, to_id=-1, message=f"Player {player_id} checks.")
            ...
            if call_needed >= gs["player_chips"][player_id]:
                ...
                self.state.add_observation(from_id=GAME_ID, to_id=-1, message=message)
    ...
    def _handle_showdown(self):
        ...
        self.state.add_observation(from_id=GAME_ID, to_id=-1, message="Showdown:\n" + "\n".join(details))
