...
def card_block(rank: str, suit: str) -> List[str]:
    """Return an ASCII art block of a card as a list of strings."""
    return [
        "┌─────────┐",
        f"│ {rank:<2}      │",
        "│         │",
...
def create_board_str(
    ...
    for line in community_row:
        output.append(f"│     {line:<65}│")

    output.append("│" + " " * 70 + "│")
    output.append("└" + "─" * 70 + "┘\n")

    # Player rows (up to 5 per row)
